...
private:
    ...
    Tpl void prf_imp_(const T& _) const { preset_(); cout << _ << end; if(exit) std::exit(0); }
    template <isVector1_ T> void prf_imp_(const T& _) const {
        i64 len_ = Size(_);
        forn(i, len_-1) { preset_(); cout << _[i] << sep; }
        if(len_) preset_(), cout << _[len_-1] << end;
        if(exit) std::exit(0);
    }
