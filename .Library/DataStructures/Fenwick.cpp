...
template <typename T = long long> struct Fenwick {
    ...
    explicit Fenwick(signed treeSize) { n = treeSize; tree = vector<T>(treeSize+10, T()); }
    void add(signed tar, const T& val) { if(tar <= 0) { return; } for(; tar<=n; tar+=tar&-tar) tree[tar] += val; }
    /// [l, r]
    T query(signed left, signed right) { if(left > right) { return T(); } left = max(left, 1); return query(right) - query(left-1); }
    /// [1, tar]
