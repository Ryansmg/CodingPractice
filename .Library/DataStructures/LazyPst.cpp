...
class LazyPst {
    vector<TreeType> tree; vector<LazyType> lazy; vector<bool> flag; vector<signed> l, r; long long ln, rn;
...
public:
    ...
    // index = [1..n]
    explicit LazyPst(const vector<TreeType>& arr) : ln(1), rn(ssize(arr)) { tree.reserve(4*rn); for(signed i=0; i<2; i++) tree.emplace_back(), lazy.emplace_back(), flag.emplace_back(0), l.emplace_back(0), r.emplace_back(0);
        init(1, ln, rn, arr); }
    struct Iter {
    ...
    struct Root {
        ...
        Root next() {
            Root ret; ret.pos = ssize(ptr->tree); ret.prvPos = pos; ret.ptr = ptr;
...
private:
    void push(long signed cur, long long s, long long e) {
        ...
        if(s != e) {
            tree.emplace_back(tree[l[cur]]), lazy.emplace_back(lazy[l[cur]]), flag.emplace_back(1), l.emplace_back(l[l[cur]]), r.emplace_back(r[l[cur]]), l[cur] = ssize(tree) - 1;
            lazy[l[cur]].update(lazy[cur], s, e);
            tree.emplace_back(tree[r[cur]]), lazy.emplace_back(lazy[r[cur]]), flag.emplace_back(1), l.emplace_back(l[r[cur]]), r.emplace_back(r[r[cur]]), r[cur] = ssize(tree) - 1;
            lazy[r[cur]].update(lazy[cur], s, e);
        }
        flag[cur] = false; lazy[cur] = LazyType();
    }
    TreeType& init(long signed cur, long long s, long long e, const vector<TreeType>& arr) {
        if(s == e) return tree[cur] = arr[s-1];
        l[cur] = ssize(tree); r[cur] = ssize(tree)+1; for(signed i=0; i<2; i++) tree.emplace_back(), lazy.emplace_back(), flag.emplace_back(0), l.emplace_back(0), r.emplace_back(0);
    ...
    void update(long signed prv, long signed cur, long long s, long long e, long long left, long long right, const UpdateType& val) {
        ...
        if(!l[cur] || l[cur] == l[prv]) tree.emplace_back(tree[l[cur]]), lazy.emplace_back(lazy[l[cur]]), flag.emplace_back(flag[l[cur]]), l.emplace_back(l[l[cur]]), r.emplace_back(r[l[cur]]), l[cur] = ssize(tree) - 1;
        if(!r[cur] || r[cur] == r[prv]) tree.emplace_back(tree[r[cur]]), lazy.emplace_back(lazy[r[cur]]), flag.emplace_back(flag[r[cur]]), l.emplace_back(l[r[cur]]), r.emplace_back(r[r[cur]]), r[cur] = ssize(tree) - 1;
