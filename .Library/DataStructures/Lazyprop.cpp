...
protected:
    vector<TreeType> tree; vector<LazyType> lazy; signed n=-1;
...
public:
    explicit Lazyprop(signed treeSize) { tree = vector<TreeType>(4*treeSize, TreeType()); lazy = vector<LazyType>(4*treeSize, LazyType()); n = treeSize; }
    explicit Lazyprop(const vector<TreeType> &a) : Lazyprop((signed) a.size()) { init(a, 1, 1, n); }
    ...
    // ret[i] == query(i+1)
    vector<TreeType> getLeafs() { vector<TreeType> ret(n);
        function<void(long long, long long, long long)> f = [&](long long p, long long s, long long e) { push(p, s, e);
            if(s == e) ret[s-1] = tree[p];
            else f(p<<1, s, (s+e)>>1), f(p<<1|1, ((s+e)>>1)+1, e); };
        f(1, 1, n); return ret; }
protected:
    TreeType& init(const vector<TreeType> &a, signed node, signed start, signed end) {
