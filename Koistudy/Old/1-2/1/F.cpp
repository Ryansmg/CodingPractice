...
struct matrix {
    ...
    bool null = false;
    explicit matrix(bool) : null(true) {}
    matrix(i64 w, i64 h) : width(w), height(h) {}
    void inputInit() {
        forn(i, height) arr.push_back(inputArr<i64>(width));
    }
    void print() {
        if(null) println("impossible!");
        else for(const vl& r : arr) printArr(r);
        println();
    }
    matrix operator+(const matrix& b) const {
        if(width != b.width || height != b.height) return matrix(true);
        matrix ret(width, height);
        ret.arr = v2l(height, vl(width));
        forn(i, height) forn(j, width) ret.arr[i][j] = arr[i][j] + b.arr[i][j];
        return ret;
    }
    matrix operator-(const matrix& b) const {
        if(width != b.width || height != b.height) return matrix(true);
    ...
    matrix operator*(const matrix& b) const {
        if(width != b.height) return matrix(true);
        matrix ret(b.width, height);
        forn(i, height) {
            vl r;
            forn(j, b.width) {
                i64 ans = 0;
                forn(k, b.height) ans += b.arr[k][j] * arr[i][k];
                r.emplace_back(ans);
            }
            ret.arr.emplace_back(r);
        }
        return ret;
    }
};
...
i32 main() {
    ...
    forn(as, q) {
        i64 a; char c; i64 b;
        cin >> a >> c >> b;
        if(c == '+') (arr[a-1] + arr[b-1]).print();
        else if(c == '-') (arr[a-1] - arr[b-1]).print();
        else (arr[a-1] * arr[b-1]).print();
    }
}
