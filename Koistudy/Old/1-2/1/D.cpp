...
public:
    I() = default;
    I(i64 i) : v(i) {}
    void in(istream& in) { in >> v; }
    operator i64() const { return v; }
    I operator--(i32) { I r(v); v--; return r; }
    void operator+=(i32 i) { v += i; }
};
istream& operator>>(istream& in, I& i) { i.in(in); return in; }
...
i32 main() {
    ...
    while(T--) {
        I a, b, c, d; cin >> c >> d;
        a += 60 + c; b += 60 + d;
        while(a/60%24!=b/60%24 || a%60 != b%60) a += 60 + c, b += 60 + d;
        printf("%02lld:%02lld\n", a/60%24, a%60);
    }
}
