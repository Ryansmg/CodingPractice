...
vl getColumn(i64 i) {
    vl ans(4); forn(j, 4) ans[j] = board[j][i];
    return ans;
}
void move(i64 dir) {
    if(dir == 1) forn(i, 4) setColumn(i, mergeLeft(getColumn(i)));
    if(dir == 2) forn(i, 4) setRow(i, mergeLeft(getRow(i)));
    if(dir == 3) forn(i, 4) setColumn(i, mergeRight(getColumn(i)));
    if(dir == 4) forn(i, 4) setRow(i, mergeRight(getRow(i)));
}
