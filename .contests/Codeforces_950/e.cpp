...
signed main() {
    ...
    while(tc--) {
        int n, m; cin >> n >> m;
        v2<int> arr(n, v<int>()), arr2 = arr;
        v<int> row, column;
        row = column = v<int>(n*m+1);
        v<set<int>> rows(n, set<int>()), columns(m, set<int>());
        forn(i, n) {
            forn(j, m) {
                int t = input();
                arr[i].push_back(t);
        ...
        forn(i, n) {
            int t = input();
            int r = row[t]; arr2[i].push_back(t);
            forn(j, m-1) {
                int t2 = input(); arr2[i].push_back(t2);
                if(!available) continue;
                available = (rows[r].find(t2)!=rows[r].end());
            }
        }
        forn(j, m) {
            if(!available) break;
            int t = arr2[0][j], c = column[t];
            forn(i, n-1) {
                if(!available) break;
                int t2 = arr2[i+1][j];
                available = columns[c].find(t2)!=columns[c].end();
            }
        }
