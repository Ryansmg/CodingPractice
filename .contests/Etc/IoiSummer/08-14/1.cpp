...
i32 main() {
    ...
    i64 n = input();
    v<i64> l(n+1), r(n+1);
    v<bool> isRoot(n+1, true);
    forf(i, 1, n) {
        cin >> l[i] >> r[i];
        isRoot[l[i]] = isRoot[r[i]] = false;
    }
    function<void(i64)> t1 = [&](i64 v) {
        if(!v) return;
        print(v, ' '); t1(l[v]); t1(r[v]);
    };
    function<void(i64)> t2 = [&](i64 v) {
        if(!v) return;
        t2(l[v]); print(v, ' '); t2(r[v]);
    };
    function<void(i64)> t3 = [&](i64 v) {
        if(!v) return;
        t3(l[v]); t3(r[v]); print(v, ' ');
    };
    forf(i, 1, n) if(isRoot[i]) {
        t1(i); println();
        t2(i); println();
        t3(i); println();
        break;
