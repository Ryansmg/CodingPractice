...
enum cdbg_{ useCerr, useCout
} cdbg=useCerr, cloc=useCout;
template <typename T>
...
i32 main() {
    ...
    v<i64> in(n+1, 0), out(n+1, 0);
    forn(i, m) {
        i64 a, b; cin >> a >> b;
        con[a].push_back(b);
        out[a]++; in[b]++;
    }
    for(auto &vec : con) sort(all(vec));
    queue<i64> q2, ans;
    priority_queue<i64, v<i64>, greater<>> q;
    forf(i, 1, n) {
        if(in[i] == 0 && out[i] == 0) q2.push(i);
        else if(in[i] == 0) q.push(i);
    }
    while(!q.empty()) {
        ans.push(t = tpop(q));
        for(ci64 i : con[t]) if(--in[i] == 0) q.push(i);
    }
    while(!ans.empty()) {
        while(!q2.empty() && q2.front() < ans.front()) cout << fpop(q2) << ' ';
        cout << fpop(ans) << ' ';
    }
    while(!q2.empty()) cout << fpop(q2) << ' ';
}
