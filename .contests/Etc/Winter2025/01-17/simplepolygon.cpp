...
Tpl64 struct GPoint {
    T x = T(), y = T();
    T distSquared(const GPoint& b) const { return sq_(x-b.x)+sq_(y-b.y); }
    f128 dist(const GPoint& b) const { return sqrt(cast<f128>(distSquared(b))); }
    bool operator<(const GPoint& b) const { return y == b.y ? x < b.x : y < b.y; }
    static auto ccwCmp(const GPoint&);
};

Tpl T product(const GPoint<T>& p1, const GPoint<T>& p2, const GPoint<T>& p3) { return (p2.x-p1.x) * (p3.y-p1.y) - (p3.x-p1.x) * (p2.y-p1.y); }
/// -1 : 시계, 0 : 직선, 1 : 반시계
Tpl i32 ccw(const GPoint<T>& p1, const GPoint<T>& p2, const GPoint<T>& p3) { T pd = product(p1, p2, p3); return pd == 0 ? 0 : ( pd < 0 ? -1 : 1 ); }

Tpl auto GPoint<T>::ccwCmp(const GPoint<T>& p) {
    return [&](const GPoint& a, const GPoint& b) -> bool {
        i32 c = ccw(p, a, b);
        return c == 0 ? p.distSquared(a) < p.distSquared(b) : c > 0;
    };
}
...
public:
    ...
    bool intersects(const GLine& l) const {
        ...
        if(ccw1 * ccw2 < 0 && ccw3 * ccw4 < 0) return true;
            ...
            if(p1.x == p2.x) return p1.y <= max(l.p1.y, l.p2.y) && min(l.p1.y, l.p2.y) <= p2.y;
...
i32 main() {
    ...
    tcRep() {
        in64(n);
        GPoint cp(inf, inf);
        v<pair<i64, GPoint<i64>>> points;
        forn(i, n) points.eb(i, GPoint(qin(2))), setMin(cp, points.back().second);
        auto cmp = GPoint<i64>::ccwCmp(cp);
        sort(points, [&](const pair<i64, GPoint<i64>> &a, const pair<i64, GPoint<i64>> &b) {
            return cmp(a.second, b.second);
        });
        i64 lastLine = n-1;
        while(ccw(cp, points[n-1].second, points[lastLine-1].second) == 0) lastLine--;
        reverse(points.begin() + lastLine, points.end());
        for(const auto &[i, P]: points) print(i, "");
        println();
    }
}
