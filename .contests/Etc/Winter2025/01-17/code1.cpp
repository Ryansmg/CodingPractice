...
i32 main() {
    fastio;
    in64(n);
    v<GPoint<i64>> points; rep(n) points.eb(qin(2));
    sort(points, GPoint<i64>::ccwCmp(min(points)));
    v<GPoint<i64>> st; st.eb(points[0]); st.eb(points[1]);
    forf(i, 2, n-1) {
        while(Size(st) >= 2 && ccw(st[Size(st)-2], st.back(), points[i]) <= 0) pop(st);
        st.eb(points[i]);
    }
    println(Size(st));
}
