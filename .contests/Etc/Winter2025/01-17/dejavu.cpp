...
i32 main() {
    ...
    if(n == 2) printExit(points[0].distSq(points[1]));
    sort(points);
    v<GPoint<i64>> up, down;
    for(const auto& p : points) {
        while(Size(up) >= 2 && ccw(up[Size(up)-2], up.back(), p) >= 0) pop(up);
        while(Size(down) >= 2 && ccw(down[Size(down)-2], down.back(), p) <= 0) pop(down);
        up.eb(p); down.eb(p);
    }
    i64 ans = 0;
    for(i64 i=0, j=Size(down)-1; i<Size(up)-1 || j>0;) {
        setMax(ans, up[i].distSq(down[j]));
        if(i == Size(up)-1) j--;
        elif(j == 0) i++;
        elif((up[i+1].y - up[i].y) * (down[j].x - down[j-1].x) > (down[j].y - down[j-1].y) * (up[i+1].x - up[i].x)) i++;
        else j--;
    }
    println(ans);
}
