...
i32 main() {
    fastio;
    in64(n, k);
    v<v<bitset<N>>> bs(3, v<bitset<N>>(k, bitset<N>()));
    v<vi> arr;
    rep(n) arr.eb(inArr<i32>(k));
    forn(i, n) {
        forn(j, k) {
            bs[0][j][i] = true;
            bs[1][j][i] = true;
    ...
    if(k == 9) {
        forn(i1, 3) forn(i2, 3) forn(i3, 3)
        forn(i4, 3) forn(i5, 3) forn(i6, 3)
        forn(i7, 3) forn(i8, 3) forn(i9, 3) {
            i64 c = (bs[i1][0] & bs[i2][1] & bs[i3][2] & bs[i4][3] & bs[i5][4] & bs[i6][5]
                    & bs[i7][6] & bs[i8][7] & bs[i9][8]).count();
