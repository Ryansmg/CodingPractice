...
i32 main() {
    fastio;
    str s = readline();
    vl lhash(1, 0), rhash(Size(s)+2, 0);
    for(char c : s) lhash.eb((lhash.back() * mod9 + c) % mod1);
    forr(i, Size(s), 1) { rhash[i] = (rhash[i+1] * mod9 + s[i-1]) % mod1; }
    i64 ans = 1, ansi = 1;
    // 홀수 길이
    forf(c, 2, Size(s)-1) {
        i64 cans = 1, cansi = c;
        for(i64 l = max(1, c + c - Size(s)), r = c; l <= r;) {
            i64 m = (l + r) >> 1, m2 = c + c - m;
            i64 mh = (lhash[c] - lhash[m-1] * pow(mod9, c-m+1, mod1) % mod1 + mod1) % mod1;
            i64 m2h = (rhash[c] - rhash[m2+1] * pow(mod9, m2+1-c, mod1) % mod1 + mod1) % mod1;
            if(mh == m2h) r = m-1, cans = m2 - m + 1, cansi = m;
            else l = m+1;
        }
        if(ans < cans) ans = cans, ansi = cansi;
    }
    // 짝수 길이
    forf(c, 1, Size(s)-1) {
        i64 cans = 2, cansi = c;
        if(s[c-1] != s[c]) continue;
        for(i64 l = max(1, c + c - Size(s) + 1), r = c; l <= r;) {
            i64 m = (l + r) >> 1, m2 = c + c - m + 1;
            i64 mh = (lhash[c] - lhash[m-1] * pow(mod9, c-m+1, mod1) % mod1 + mod1) % mod1;
            i64 m2h = (rhash[c+1] - rhash[m2+1] * pow(mod9, m2-c, mod1) % mod1 + mod1) % mod1;
            if(mh == m2h) r = m-1, cans = m2 - m + 1, cansi = m;
            else l = m+1;
        }
        if(ans < cans) ans = cans, ansi = cansi;
    }
    forf(i, ansi-1, ansi+ans-2) print(s[i]);
    println();
}
