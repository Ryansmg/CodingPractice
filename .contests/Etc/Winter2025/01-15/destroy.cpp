...
i32 main() {
    fastio;
    in64(n, k);
    // x : 1 ~ 500, y : 501 ~ 1000, source:0, sink:1001
    Flow flow(1001, 0, 1001);
    rep(k) {
        in64(x, y);
        flow.addEdge(x, y+500, inf);
    }
    forf(i, 1, n) flow.addEdge(0, i, 1), flow.addEdge(i+500, 1001, 1);
    flow.run();
    println(flow.maxFlow);
}
