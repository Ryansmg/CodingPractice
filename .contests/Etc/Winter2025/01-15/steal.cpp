...
i32 main() {
    ...
    in64(n, m);
    DGraph g(n);
    rep(m) g.makeDEdge(qin(2));
    vl money(1, -INF); inArr(money, n);
    in64(s, p);
    vb isExit(n+1, false); rep(p) isExit[qin()] = true;
    v2l sccs = g.getScc({0});
    vl sccId(n+1, -1);
    i64 newN = Size(sccs); DGraph newG(newN-1);
    forn(i, newN) for(ci64 j : sccs[i]) sccId[j] = i;
    vl newMoney(newN, 0);
    vb newExit(newN, false), canVisit(newN, false);
    forf(i, 1, n) {
        for(const auto& [I, j] : g.child[i]) {
            if(sccId[i] == sccId[j]) continue;
            newG.makeDEdge(sccId[i], sccId[j]);
        }
        if(isExit[i]) newExit[sccId[i]] = true;
        if(i == s) canVisit[sccId[i]] = true;
        newMoney[sccId[i]] += money[i];
    }
    i64 ans = 0;
    vl mxm(newN, 0);
    for(ci64 i : Dag(newG).topologySort()) {
        if(!canVisit[i]) continue;
        mxm[i] = newMoney[i];
        for(const auto& [j, I] : newG.parent[i]) setMax(mxm[i], newMoney[i] + mxm[j]);
        for(const auto& [I, j] : newG.child[i]) canVisit[j] = true;
        if(newExit[i]) setMax(ans, mxm[i]);
    }
    println(ans);
}
