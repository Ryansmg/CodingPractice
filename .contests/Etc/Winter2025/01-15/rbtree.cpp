...
i32 main() {
    ...
    DGraph g(n*2+1);
    rep(m) {
        i64 a, b, c; char A, B, C;
        input(a, A, b, B, c, C);
        a = A=='B' ? (a)*2+1 : a*2;
        b = B=='B' ? (b)*2+1 : b*2;
        c = C=='B' ? (c)*2+1 : c*2;
        g.makeDEdge(a^1, b); g.makeDEdge(a^1, c);
        g.makeDEdge(b^1, a); g.makeDEdge(b^1, c);
        g.makeDEdge(c^1, a); g.makeDEdge(c^1, b);
    }
    v2l sccs = g.getScc({0});
    vl sccId(n*2+2, -1);
    forn(i, Size(sccs)) for(ci64 j : sccs[i]) sccId[j] = i;
    forf(i, 1, n) if(sccId[2*i] == sccId[2*i+1]) printExit(-1);
    v<char> ans(n, -1);
    for(const vl& scc : sccs) {
        for(ci64 i : scc) {
            if(ans[i/2-1] == -1) ans[i/2-1] = (i&1) ? 'R' : 'B';
        }
    }
    printfln(.sep="")(ans);
}
