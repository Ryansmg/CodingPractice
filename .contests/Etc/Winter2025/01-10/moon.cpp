...
int main() {
    int i, j;
    int moon_count_by_init;
    int max_moon_count_by_query = 0;
    ...
    moon_count = 0;
    init(N, A);
    moon_count_by_init = moon_count;

    for (j = 0; j < Q; ++j) {
        moon_count = 0;
        printf("%d\n", query(L[j], R[j]));
        if (max_moon_count_by_query < moon_count) {
            max_moon_count_by_query = moon_count;
        }
    }

    fprintf(stderr, "number of calls in init : %d\n",
            moon_count_by_init);
    fprintf(stderr, "maximum number of calls in query : %d\n",
            max_moon_count_by_query);
...
struct tr {
    i32 i=0; bool isDummy = true; // isDummy : 항등원 표시
    tr()=default; // 항등원 생성
    explicit tr(i32 j) : i(j), isDummy(false) {}
    tr operator+(const tr& b) const {
        if(isDummy) {return b;} if(b.isDummy) {return *this;}
        return tr(moon(i, b.i));
    }
};

Segtree<tr> seg;

void init(i32 N, std::vector<i32> A) {
    seg = Segtree<tr>(castVec<tr>(A));
}

i32 query(i32 L, i32 R) {
    return seg.query(L+1, R+1).i;
}
