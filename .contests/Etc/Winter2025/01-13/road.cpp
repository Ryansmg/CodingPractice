...
template <typename T, typename Func> v<T> funVec(ci64 len, const Func& f) { v<T> ret; forn(i, len) { ret.pb(f(i)); } return ret; }
...
i32 main() {
    ...
    i64 ans = -1;
    fun<void(i64, i64, i64)> f = [&](i64 vis, i64 cur, i64 w) {
        if(cur == n) { ans = max(ans, w); return; }
        if(vis & (1 << cur)) { f(vis, cur+1, w); return; }
        forf(i, cur+1, n) if(!(vis & (1 << i))) f(vis | (1<<i), cur+1, w+d[cur][i]);
    };
    f(0, 1, 0);
    println(ans);
}
