...
struct tr {
    i64 v = -INF;
    tr operator+(const tr& b) const { return {max(v, b.v)}; }
};
...
i32 main() {
    ...
    v<ii> qrs; vl ans;
    in64(n);
    Segtree<tr> seg(castVec<tr>(inArr(n)));
    auto cmp = [&](ci64 A, ci64 B) -> bool {
        ii a = qrs[A], b = qrs[B];
        bool swapped = false;
        if(a[0] > b[0] || (a[0]==b[0] && a[1] < b[1])) swap(a, b), swapped = true;
        auto f = [&](bool ret) { return swapped ? !ret : ret; };
        if(a[1] < b[0]) return f(seg.query(a[0], a[1]).v < seg.query(b[0], b[1]).v);
        if(b[1] <= a[1]) return f(false);
        return f(seg.query(a[0], b[0]-1).v < seg.query(a[1]+1, b[1]).v);
    };
    in64(q); qrs.pb({0, 0});
    forf(i, 1, q) ans.eb(i);
    rep(q) qrs.pb({qin(2)});
