...
class DynamicSeg {
    ...
    vi l, r;
public:
    explicit DynamicSeg(i64 size) : n(size) {
        tree.eb(); l.eb(-1); r.eb(-1);
    }
    void update(i64 tar, const T& val) { update(0, 1, n, tar, val); }
    T query(i64 left, i64 right) { return query(0, 1, n, left, right); }
private:
    ...
    T query(i32 p, i64 s, i64 e, i64 ql, i64 qr) {
        ...
        return query(l[p], s, (s+e)/2, ql, qr) + query(r[p], (s+e)/2+1, ql, qr);
    }
};
...
struct MarsSeg {
    vl tree, cnt; i64 n;
    vi l, r;
    explicit MarsSeg(i64 size) : n(size) {
        tree.eb(); l.eb(-1); r.eb(-1); cnt.eb();
    }
    void update(i64 v, i64 ql, i64 qr) { update(0, 1, n, ql, qr, v); }
    i64 update(i32 p, i64 s, i64 e, i64 ql, i64 qr, const i64& v) {
        if(e < ql || qr < s) return tree[p];
        if(s == e) {
            cnt[p] += v;
            return tree[p] = cnt[p] ? 1 : 0;
        }
        if(ql <= s && e <= qr) cnt[p] += v;
        else {
            if(l[p] == -1) l[p] = Size(tree), tree.eb(), cnt.eb(), l.eb(-1), r.eb(-1);
            if(r[p] == -1) r[p] = Size(tree), tree.eb(), cnt.eb(), l.eb(-1), r.eb(-1);
            update(l[p], s, (s+e)/2, ql, qr, v), update(r[p], (s+e)/2+1, e, ql, qr, v);
        }
        if(cnt[p]) return tree[p] = e-s+1;
        return tree[p] = (l[p] == -1 ? 0 : tree[l[p]]) + (r[p] == -1 ? 0 : tree[r[p]]);
    }
};

i32 main() {
    fastio;
    MarsSeg seg(1000000010);
    struct query {
        i64 x, y1, y2, d;
