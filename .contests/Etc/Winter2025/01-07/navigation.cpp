...
i32 main() {
    ...
    in64(n);
    v<iii> edges;
    rep(n-1) edges.pb({qin(3)});
    Graph g(n); for(const auto& [a, b, c] : edges) g.addUEdge({a, b});
    Tree t(1, g); t.initHld();
    vl arr(n, 0);
    for(const auto& [a, b, c] : edges) {
        if(a != 1 && t.par(a) == b) arr[t.in[a]-1] = c;
        else arr[t.in[b]-1] = c;
    }
    Segtree lp(arr);
    inRep() {
        i64 ans = 0;
        auto f = [&](i64 l, i64 r) { ans += lp.query(l, r); };
        i64 lca = t.hld(qin(2), f);
        println(ans - lp.query(t.in[lca]));
    }
}
