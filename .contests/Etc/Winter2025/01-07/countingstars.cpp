...
public:
    ...
    void update(i64 tar, const T& val) { update(0, ln, rn, tar, val); }
    T query(i64 left, i64 right) { return query(0, ln, rn, left, right); }
private:
    T& update(i32 p, i64 s, i64 e, i64 t, const T& v) {
        if(s == e) return tree[p] = tree[p] + v;
        if(t <= (s + e) / 2) {
            if(l[p] == -1) l[p] = Size(tree), tree.eb(), l.eb(-1), r.eb(-1);
            return tree[p] = update(l[p], s, (s+e)/2, t, v) + (r[p] == -1 ? T() : tree[r[p]]);
        }
        if(r[p] == -1) r[p] = Size(tree), tree.eb(), l.eb(-1), r.eb(-1);
        return tree[p] = (l[p] == -1 ? T() : tree[l[p]]) + update(r[p], (s+e)/2+1, e, t, v);
    }
...
class Dynamic2dSeg {
    ...
    vi l, r;
public:
    Dynamic2dSeg(i64 lxi, i64 rxi, i64 lyi, i64 ryi) : lx(lxi), rx(rxi), ly(lyi), ry(ryi) {
        tree.eb(ly, ry); l.eb(-1); r.eb(-1);
    }
    void update(i64 tx, i64 ty, const T& val) {
        update(0, lx, rx, tx, ty, val);
    }
    T query(i64 left_x, i64 right_x, i64 left_y, i64 right_y) {
        return query(0, lx, rx, left_x, right_x, left_y, right_y);
    }
private:
    void update(i32 p, i64 sx, i64 ex, i64 tx, i64 ty, const T& val) {
        tree[p].update(ty, val);
        if(sx == ex) return;
        i64 mx = (sx + ex) / 2;
        if(tx <= mx) {
            if(l[p] == -1) l[p] = Size(tree), tree.eb(ly, ry), l.eb(-1), r.eb(-1);
            update(l[p], sx, mx, tx, ty, val);
        } else {
            if(r[p] == -1) r[p] = Size(tree), tree.eb(ly, ry), l.eb(-1), r.eb(-1);
            update(r[p], mx+1, ex, tx, ty, val);
        }
    }
...
i32 main() {
    fastio;
    Dynamic2dSeg seg(0, 100000, 0, 100000);
    in64(n, q);
    v<iii> updates; vl cx, cy;
    rep(n) {
        in64(a, b, c);
        cx.eb(a); cy.eb(b);
        updates.pb({a, b, c});
    ...
    forn(i, n) seg.update(idx(updates[i][0], cx), idx(updates[i][1], cy), updates[i][2]);
    i64 ans = 0;
    rep(q) {
        in64(a, b, c, d);
        i64 A = ((a^ans)%1000000000)+1;
        i64 B = ((b^ans)%1000000000)+1;
        i64 C = ((c^ans)%1000000000)+1;
        i64 D = ((d^ans)%1000000000)+1;
        a = min(A, B), b = max(A, B), c = min(C, D), d = max(C, D);
        a = lb(cx, a) - cx.begin();
        b = prev(ub(cx, b)) - cx.begin();
        c = lb(cy, c) - cy.begin();
        d = prev(ub(cy, d)) - cy.begin();
        println(ans = seg.query(a, b, c, d));
    }
}
