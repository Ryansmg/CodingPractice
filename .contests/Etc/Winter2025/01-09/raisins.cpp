...
vl mem(9000000, -1);
PrefixSum2d ps;
i64 solve(i64 ly, i64 ry, i64 lx, i64 rx) {
    i64 cur = ly*50*50*50 + ry*50*50 + lx*50 + rx;
    i64 ret = INF;
    if(ly == ry && lx == rx) return 0;
    if(mem[cur] != -1) return mem[cur];
    forf(i, lx, rx-1) ret = min(ret, solve(ly, ry, lx, i) + solve(ly, ry, i+1, rx));
    forf(i, ly, ry - 1) ret = min(ret, solve(ly, i, lx, rx) + solve(i + 1, ry, lx, rx));
    return mem[cur] = ret + ps(ly, ry, lx, rx);
}
...
i32 main() {
    ...
    rep(n) {
        arr.eb();
        rep2(m) arr.back().eb(input());
    }
    ps = PrefixSum2d(arr);
    println(solve(0, n-1, 0, m-1));
}
