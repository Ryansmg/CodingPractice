...
i32 main() {
    ...
    v<map<i64, i64>> dp(n, map<i64, i64>());
    i64 m = inf, M = -inf;
    for(ci64 i : arr) {
        m = min(m, i);
        M = max(M, i);
    ...
    forr(i, n-1, 0) {
        forf(j, i+1, n-1) {
            i64 prv = arr[i] + arr[i] - arr[j];
            if(prv < m || prv > M) {
                auto ptr1 = dp[j].find(arr[j]-arr[i]);
                i64 l = ((ptr1==dp[j].end()?0:ptr1->second) + 1) % mod;
                ans = (ans + l) % mod;
                continue;
            }
            auto ptr1 = dp[j].find(arr[j]-arr[i]);
            i64 l = ((ptr1==dp[j].end()?0:ptr1->second) + 1) % mod;
            auto ptr = dp[i].find(arr[j] - arr[i]);
            if(ptr == dp[i].end()) {
                dp[i].insert({arr[j] - arr[i], l});
            } else {
                ptr->second = (ptr->second + l) % mod;
            }
            ans = (ans + l) % mod;
        }
    }
