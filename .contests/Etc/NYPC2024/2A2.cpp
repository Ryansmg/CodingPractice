...
i32 main() {
    ...
    forn(i, m) {
        i64 r1, c1, r2, c2;
        cin >> r1 >> c1 >> r2 >> c2;
        if(r1 == r2) r.push_back(r1);
        else if(c1 == c2) c.push_back(c1);
        else if(r1+c1 == r2+c2) rpc.push_back(r1+c1);
        else rmc.push_back(r1-c1);
    }
    sort(r); sort(c); sort(rpc); sort(rmc);
    forn(qi, q) {
        i64 r1, c1, r2, c2;
        cin >> r1 >> c1 >> r2 >> c2;
        // r
        auto rlb1 = lower_bound(all(r), r1);
        if(rlb1 != r.end() && *rlb1 == r1) { println(0); continue; }
        auto rlb2 = lower_bound(all(r), r2);
        if(rlb2 != r.end() && *rlb2 == r2) { println(0); continue; }
        if(rlb1 != rlb2) { println(0); continue; }

        // c
        auto clb1 = lower_bound(all(c), c1);
        if(clb1 != c.end() && *clb1 == c1) { println(0); continue; }
        auto clb2 = lower_bound(all(c), c2);
        if(clb2 != c.end() && *clb2 == c2) { println(0); continue; }
        if(clb1 != clb2) { println(0); continue; }

        // rpc
        auto rpclb1 = lower_bound(all(rpc), r1+c1);
        if(rpclb1 != rpc.end() && *rpclb1 == r1+c1) { println(0); continue; }
        auto rpclb2 = lower_bound(all(rpc), r2+c2);
        if(rpclb2 != rpc.end() && *rpclb2 == r2+c2) { println(0); continue; }
        if(rpclb1 != rpclb2) { println(0); continue; }

        // rmc
        auto rmclb1 = lower_bound(all(rmc), r1-c1);
        if(rmclb1 != rmc.end() && *rmclb1 == r1-c1) { println(0); continue; }
        auto rmclb2 = lower_bound(all(rmc), r2-c2);
        if(rmclb2 != rmc.end() && *rmclb2 == r2-c2) { println(0); continue; }
        if(rmclb1 != rmclb2) { println(0); continue; }
        println(1);
    }
}
