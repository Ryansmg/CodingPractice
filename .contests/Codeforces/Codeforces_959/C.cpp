...
i32 main() {
    ...
    forf(tci, 1, T) {
        ...
        v<i64> arr2(n); arr2[0] = arr[0];
        forf(i, 1, n-1) arr2[i] = arr2[i-1]+arr[i];
        v<i64> w(n+5, 1);
        forn(i, n) {
            i64 off = i?arr2[i-1]:0;
            auto ptr = upper_bound(arr2.begin()+i, arr2.end(), x+off);
            if(ptr != arr2.end()) {
                ans += w[i];
                w[ptr-arr2.begin()+1] += w[i];
            }
        }
        println(n*(n-1)/2+n-ans);
    }
}
