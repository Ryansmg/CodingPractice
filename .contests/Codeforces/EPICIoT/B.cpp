...
i32 main() {
    ...
    while(T--) {
        i64 n = input();
        v<i64> alice, bob;
        inputArr(alice, n); inputArr(bob, n);
        i64 al = 0, ar = n-1, bl = 0, br = n-1;
//        while(al <= ar) {
//            bool b = true;
//            if(alice[al] == bob[bl] && alice[ar] == bob[br]) { al++; bl++; ar--; br--; b = false; }
//            if(alice[al] == bob[br] && alice[ar] == bob[bl]) { al++; bl++; ar--; br--; b = false; }
//            if(b) break;
//        }
        if(alice[al] == bob[bl] && alice[ar] == bob[br] || alice[al] == bob[br] && alice[ar] == bob[bl]) println("Bob");
        else println("Alice");
    }
}
