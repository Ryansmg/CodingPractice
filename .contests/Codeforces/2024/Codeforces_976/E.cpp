...
i32 main() {
    ...
    inRep() {
        ...
        p[a[0]] = b[0]; p[0] = 10000 - b[0];
        forf(i, 1, n-1) {
            vl nxt(1024, 0);
            forf(j, 0, 1023) {
                nxt[j^a[i]] += p[j] * b[i] % M;
                nxt[j] += p[j] * (10000 - b[i]) % M;
                nxt[j^a[i]] %= M;
                nxt[j] %= M;
            }
            p = nxt;
        }
        i64 twop = n*4, fivep = n*4;
        i128 P = 0;
        forf(i, 1, 1023) P += i * i * p[i];
        while(P % 2 == 0 && twop > 0) P /= 2, twop--;
        while(P % 5 == 0 && fivep > 0) P /= 5, fivep--;
        i64 Q = 1;
        rep(twop) Q = Q * 2 % M;
        rep(fivep) Q = Q * 5 % M;
        println(toi64(P * modInv(Q, M) % M));
    }
}
