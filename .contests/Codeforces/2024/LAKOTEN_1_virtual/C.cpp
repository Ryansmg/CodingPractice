...
i32 main() {
    ...
    inRep() {
        i64 n = in();
        i64 cnt = 0;
        vl par(n+1); iota(all(par), 0);
        vl a = inArr(n), b = inArr(n);
        func<i64(i64)> find = [&](i64 i) -> i64 {
            if(par[i] == i) return i;
            return par[i] = find(par[i]);
        };
        auto merge = [&](i64 a, i64 b) {
            i64 r1 = find(a), r2 = find(b);
            if(r1 == r2) return;
            else par[r1] = r2;
        };
        forn(i, n) merge(a[i], b[i]);
        forf(i, 1, n) cnt += par[i] == i;
        println(pow_(2LL, cnt, 1000000007LL));
    }
}
