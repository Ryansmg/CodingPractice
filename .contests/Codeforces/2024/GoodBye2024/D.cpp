...
i32 main() {
    ...
    tcRep() {
        ...
        vl sa = sorted(a), sb = sorted(b);
        i64 ans = 1;
        forn(i, n) ans = ans * min(sa[i] , sb[i]) % mod;
        print(ans, " ");
        rep(q) {
            in64(o, x); x--;
            if(o == 1) {
                auto iter = prev(ub(sa, a[x]));
                i64 idx = iter - sa.begin();
                ans = ans * pow_(min(sa[idx], sb[idx]), mod-2, mod) % mod;
                sa[idx]++; a[x]++;
                ans = ans * min(sa[idx], sb[idx]) % mod;
            } else {
                auto iter = prev(ub(sb, b[x]));
                i64 idx = iter - sb.begin();
                ans = ans * pow_(min(sa[idx], sb[idx]), mod-2, mod) % mod;
                sb[idx]++; b[x]++;
                ans = ans * min(sa[idx], sb[idx]) % mod;
            }
