...
using pll = pair<i64, i64>; using pii = pair<i32, i32>;
...
pll ans(i64 l, i64 r, i64 n, i64 k) {
    if(r-l+1 < k) return {0, 0};
    i64 m = (l+r)/2;
    if((r-l+1)%2==0) {
        auto [s, cnt] = ans(l, m, n, k);
        return {s * 2 + cnt * m , cnt*2 };
    } else {
        if(l == r) return {m, 1};
        auto [s, cnt] = ans(l, m-1, n, k);
        return {s * 2 + cnt * m + m , cnt*2+1 };
    }
}
