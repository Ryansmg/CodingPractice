...
i32 main() {
    ...
    tcRep() {
        ...
        rep(n) {
            in64(a, b);
            if(a == b) neces[a]++, ncp[a] = 1;
            queries.pb({a, b});
        }
        forf(i, 1, 2*n+9) ncp[i] += ncp[i-1];
        forn(i, n) {
            if(!neces[queries[i][0]] || !neces[queries[i][1]]) {
                print("1"); continue;
            }
            if(queries[i][0] == queries[i][1]) {
                print(neces[queries[i][0]] == 1 ? 1 : 0);
                continue;
            }
            if(ncp[queries[i][1]] - ncp[queries[i][0]] == queries[i][1] - queries[i][0]) {
                print(0); continue;
            }
            print(1);
        }
