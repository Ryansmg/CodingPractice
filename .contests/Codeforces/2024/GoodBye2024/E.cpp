...
i32 main() {
    ...
    tcRep() {
        ...
        v2l adj(n+1, vl());
        vl dist(n+1, inf);
        i64 cnt0 = 0, cnt1 = 0, cnt2 = 0;
        rep(n-1) {
            in64(a, b);
            adj[a].eb(b); adj[b].eb(a);
        }
        queue<pair<i64, i64>> q;
        forf(i, 1, n) if(Size(adj[i]) == 1) {
            dist[i] = 0;
            q.emplace(0, i);
        }
        while(!q.empty()) {
        ...
        forf(i, 1, n) {
            ...
            bool flag = true;
            for(ci64 j : adj[i]) if(dist[j] != 1) { flag = false; break; }
            if(dist[i] > 1) adj11cnt2 += flag;
            else adj11cnt1 += flag;
        }
        lprintvar(cnt0, cnt1, cnt2, adj11cnt1, adj11cnt2);
        println(cnt0 * (cnt1 + cnt2) + (adj11cnt1 + adj11cnt2) * cnt2 - adj11cnt2 * adj11cnt2 + adj11cnt2 * (cnt2 - adj11cnt2));
    }
}
