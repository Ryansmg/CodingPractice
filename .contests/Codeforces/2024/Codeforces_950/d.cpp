...
signed main() {
    ...
    while(tc--) {
        ...
        v<int> gcdr;
        forf(i, 1, n-1) gcdr.push_back(gcd_(arr[i-1], arr[i]));
        int prev = -INF, ndec = 0, idx = -1, fidx, fused;
        bool used = false, answered = false;
        for(int i : gcdr) {
            idx++;
            if(prev <= i) {
                if(ndec!=0 && used) {
        ...
        if(!answered) {
            if(fused == 2) {
                int g = gcd_(arr[fidx-1], arr[fidx+1]);
                if(!(gcdr[fidx-2] <= g && (edge || g <= gcdr[fidx+1]))) {
                    cout << "NO\n"; continue;
                }
            }
            if(fused == 1) {
                ...
                int g;
                if(!edge) {
                    g = gcd_(arr[fidx], arr[fidx + 2]);
                    if ((gcdr[fidx - 1] <= g && g <= gcdr[fidx + 2])) succeed = true;
                }
                g = gcd_(arr[fidx-1], arr[fidx+1]);
                if(gcdr[fidx-2] <= g && (edge || g <= gcdr[fidx+1])) succeed = true;
                if(!succeed) {
                    cout << "NO\n"; continue;
                }
            }
