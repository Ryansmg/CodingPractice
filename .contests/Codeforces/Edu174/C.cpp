...
struct ModInt {
    long long v = 0;
    ModInt() = default;
    ModInt(long long val) : v((val % mod + mod) % mod) {} // NOLINT(*-default-constructor)
    explicit operator long long() { return v; }
    ModInt& operator=(const ModInt& b) = default;
    ModInt& operator=(long long b) { v = (b % mod + mod) % mod; return *this; }
...
i32 main() {
    tcRep() {
        in64(n);
        vl arr(n, cin);
        ModInt<mod9> curOneCnt = 0, waitingOne = 0, curAns = 0, ans = 0;
        for(ci64 i : arr) {
            if(i == 1) waitingOne++;
            elif(i == 2) {
                curAns += waitingOne;
                curOneCnt += waitingOne;
                waitingOne = 0;
                curAns *= 2;
            }
            else ans += curAns - curOneCnt;
        }
        println(ans);
    }
}
