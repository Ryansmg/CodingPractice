...
i32 main() {
    tcRep() {
        str s = inStr();
        i64 l = 0, r = Size(s)-1;
        while(l <= r && s[l] == s[Size(s)-l-1]) l++, r--;
        if(r - l + 1 == 0) println(0);
        else {
            i64 m = Size(s)/2;
            i64 ans = INF;

            {
                ...
                vi cnt(26, 0);
                vi tot(26, 0);
                forf(i, l, r) tot[s[i]-'a']++;
                forn(i, 26) if(!tot[i]) cur++;
                i64 i = l;
                while(cur < 26) {
                    if(++cnt[s[i]-'a'] == tot[s[i]-'a']/2) cur++;
                    if(cur < 26) i++;
                }
                setMin(ans, i - l + 1);
                cnt = vi(26, 0); cur = 0; i = r;
                forn(k, 26) if(!tot[k]) cur++;
                while(cur < 26) {
                    if(++cnt[s[i]-'a'] == tot[s[i]-'a']/2) cur++;
                    if(cur < 26) i--;
                }
            ...
            while(m <= r) {
                if(s[m] == s[Size(s)-m-1]) s[m] = s[Size(s)-m-1] = '+';
                m++;
            }
            i64 cur = 0;
            vi cnt(26, 0);
            vi tot(26, 0);
            forf(i, l, r) if(s[i] != '+') tot[s[i]-'a']++;
            forn(i, 26) if(!tot[i]) cur++;
            i64 i = l;
            while(cur < 26) {
                if(s[i] != '+' && ++cnt[s[i]-'a'] == tot[s[i]-'a']/2) cur++;
                if(cur < 26) i++;
            }
            setMin(ans, i - l + 1);
            cnt = vi(26, 0); cur = 0; i = r;
