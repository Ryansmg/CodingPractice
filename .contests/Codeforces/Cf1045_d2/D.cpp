...
i32 main() {
    tcRep() {
        in64(n);
        v2l adj = adj_list(n, n - 1);
        i64 max_degree = 0;
        for(const auto& i : adj) setMax(max_degree, i.size());
        if(max_degree <= 2) {
            println(-1);
            continue;
        }

        fun<i64(i64)> furthest_node = [&](i64 node) {
            i64 max_dep = 0, ans = node;
            fun<void(i64, i64, i64)> dfs = [&](i64 cur, i64 par, i64 cur_dep) {
                if(cur_dep > max_dep) max_dep = cur_dep, ans = cur;
                for(ci64 i : adj[cur]) {
                    if(i == par) continue;
                    dfs(i, cur, cur_dep + 1);
                }
        ...
        i64 a = furthest_node(1);
        i64 b = furthest_node(a);

        vl ab_path;

        fun<bool(i64, i64)> find_path = [&](i64 cur, i64 prv) {
            ab_path.pb(cur);
            if(cur == b) return true;
            for(ci64 i : adj[cur]) {
                if(i == prv) continue;
                if(find_path(i, cur)) return true;
            }
            ab_path.pop_back();
            return false;
        };
        bool t = find_path(a, -1);
        assert(t);
        i64 l = 0, r = ssize(ab_path) - 1;
        while(adj[ab_path[l]].size() <= 2) l++;
        while(adj[ab_path[r]].size() <= 2) r--;
        i64 left_ans = r, right_ans = ssize(ab_path) - 1 - l;
        if(left_ans <= right_ans) println(ab_path[r+1], ab_path[r], ab_path[r-1]);
        else println(ab_path[l-1], ab_path[l], ab_path[l+1]);
    }
}
