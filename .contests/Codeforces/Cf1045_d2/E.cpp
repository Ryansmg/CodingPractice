...
i32 main() {
    tcRep() {
        in64(n);
        vl ans(n + 1, -1);
        vl b(n + 3, 0); // 현재의 i에 던지면 튕기는 횟수
        flprintln("throw", n - 1);
        if(input() == 1) ans[n-1] = 2;
        else ans[n-1] = 1;
        flprintln("swap", n - 1);
        flprintln("throw", n - 1);
        if(input() == 1) ans[n] = 2;
        ...
        forr(i, n - 2, 2) {
            if(b[i+1] != b[i+2]) {
                flprintln("throw", i);
                in64(t);
                if(t == b[i+1] + 1) ans[i] = 1;
                else ans[i] = 2;
                b[i] = t;
            } else {
                flprintln("throw", i-1);
                in64(t);
                if(t == b[i+1] + 1) ans[i-1] = 2, b[i-1] = b[i+1] + 1;
                else {
                    assert(t == b[i+1] + 2);
                    ans[i-1] = 1;
                    b[i-1] = b[i+1] + 2;
