...
i32 main() {
    ...
    tcRep() {
        in64(n, q);
        vl w = inArr(n);
        vec<vl> leftBit(30, vl(n, -1)); // [i] : 1<<i를 가지고 있는 가장 가까운 왼쪽 인덱스, 왼쪽에 없으면 -1
        forn(i, n) {
            forn(j, 30) {
                leftBit[j][i] = ((1 << j) & w[i]) ? i : (i ? leftBit[j][i-1] : -1);
            }
        }
        vl xorW = w; // xor 누적합
        forn(i, n-1) xorW[i+1] ^= xorW[i];
        rep(q) {
            in64(v);
            i64 idx = n - 1; // 얘를 먹고 싶다
            i64 curBit = 29; // 현재 최상위 비트 위치
            while(!(v & (1 << curBit))) curBit--;
            while(true) {
                if(w[idx] > v) break;
                ...
                i64 wall = -1; // 이새끼는못넘음
                forr(i, 29, curBit+1) setMax(wall, leftBit[i][idx-1]);
                if(wall >= leftBit[curBit][idx]) {
                    idx = wall; break;
                } else {
                    v ^= xorW[idx-1];
                    idx = leftBit[curBit][idx];
                    v ^= xorW[idx];
                    assert(v & (1 << curBit));
                }
            }
