...
i32 main() {
    ...
    tcRep() {
        in64(n);
        vl p(1, -1); inVec(p, n);
        vl idx(n+1, -1); forf(i, 1, n) idx[p[i]] = i;
        vl c(1, -1); inVec(c, n);
        UF l(n); UF r(n);
        i64 curC = c[1], curL = 1;
        forf(i, 1, n) {
            if(curC != c[i]) {
                forf(j, curL, i-1) l[j] = curL, r[j] = i-1;
                curL = i; curC = c[i];
            }
        }
        forf(j, curL, n) l[j] = curL, r[j] = n;
        map cnt;
        forf(i, 1, n) cnt[l(i)]++;
        i64 ans = 1;
        forf(len, 1, n) {
            i64 i = idx[len]; auto iter = cnt.find(l(i));
            ans *= iter->second; ans %= mod9; iter->second--;
            if(iter->second == 0) {
                i64 a = l(i) - 1, b = r(i) + 1;
                while(a >= 1 && !cnt[l(a)]) l[i] = a, a = l(a) - 1;
                while(b <= n && !cnt[l(b)]) r[i] = b, b = r(b) + 1;
                if(a < 1 || b > n || c[a] != c[b]) continue;
                cnt[l(a)] += cnt[l(b)];
                l[b] = a; r[a] = b;
            }
        }
