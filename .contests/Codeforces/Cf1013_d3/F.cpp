...
vi sieve2(int n) {
    ...
    forf(i, 2, n) if(r[i]) {
        ret.push_back(i);
        for(i64 j = i+i; j<=n; j+=i) r[j] = false;
    }
    return ret;
}
...
i32 main() {
    tcRep() {
        ...
        i64 x = d-1;
        vec<str> s(n);
        forr(i, n-1, 0) s[i] = inStr();
        vec<ModInt<mod9>> nxt(m+1), cur(m+1), nxt2(m+1);
        forn(i, m) cur[i] = s[0][i] == 'X', nxt2[max(0, i-d)] += s[0][i] == 'X', nxt2[min(m-1, i+d)+1] -= s[0][i] == 'X';
        nxt2.accumulate();
//        forn(i, m) cur[i] += nxt2[i];
        cur = nxt2;
        forf(i, 1, n-1) {
            forn(j, m) if(s[i-1][j] == 'X') {
                nxt[max(0, j-x)] += cur[j];
                nxt[min(m-1, j+x)+1] -= cur[j];
            }
            nxt.accumulate();
            nxt2 = vec<ModInt<mod9>>(m+1, 0);
            forn(j, m) if(s[i][j] == 'X') {
                nxt2[max(0, j-d)] += nxt[j];
                nxt2[min(m-1, j+d)+1] -= nxt[j];
            }
            nxt2.accumulate();
            nxt = nxt2;
            cur = nxt; nxt = vec<ModInt<mod9>>(m+1, 0);
        }
        ModInt<mod9> ans = 0;
        forn(i, m) if(s[n-1][i] == 'X') ans += nxt2[i];
        println(ans);
    }
}
