...
inline signed popcount(long long v) { return popcount((unsigned long long) v); }
...
#define instr(...) string __VA_ARGS__; input<string>(__VA_ARGS__)
...
int main() {
    tcRep() {
        ...
        vector<i64> arr(n);
        forn(i, n) arr[i] = get();
        vector<i64> arr1(n, -2147483647);
        segtree<node> seg1(0, n + 1);
        forn(i, n) {
            i64 t = seg1.query(arr[i] + 1, n + 1).val;
            if(t != -2147483647) arr1[i] = t;
            seg1.set(arr[i], i);
        }
        vector<i64> arr2(n, -2147483647);
        segtree<node> seg2(0,  n + 1);
        forn(i, n) {
            i64 t = seg2.query(arr[i] + 1, n + 1).val;
            if(t != -2147483647) arr2[i] = t;
            seg2.set(arr[i], arr1[i]);
        }
        vector<node> arr3(n);
        forn(i, n) arr3[i] = node(arr2[i]);
        segtree<node> seg3(arr3);
        rep(k) {
            in64(l, r);
            if(seg3.query(l, r).val + 1 >= l) ln("NO");
            else ln("YES");
        }
    }
}
