...
i32 main() {
    tcRep() {
        ...
        // (-I, I)
        i64 minD = 2147483647, x = 0, y = 0;

        // (-I, I)
        rep(n) {
            in64(a, b);
            i64 d = abs(-I - a) + abs(I - b);
            if(d < minD) x = a, y = b, minD = d;
            elif(d == minD && x < a) x = a, y = b;
        }
        i64 prvD = -1;

        i64 dx = -3 * I, dy = 3 * I;

        rep(3) flprintln("? U", I), prvD = input();
        rep(3) flprintln("? L", I), prvD = input();
        ...
        while(true) {
            ...
            // 현재 y좌표 : y + up
            // 현재 x좌표 : x + (curD - up)
            flprintln("!", x + curD - up - dx, y + up - dy);
            break;
        }
    }
}
