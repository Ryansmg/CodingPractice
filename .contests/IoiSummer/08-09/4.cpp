...
i32 main() {
    fastio;
    i64 n, q; cin >> n >> q;
    v<i64> arr = sorted_copy(inputArr<i64>(n));
    v<i64> arr2 = arr; prefixSum(arr2);
    ...
    forn(i, q) {
        i64 l, r; cin >> l >> r;
        if(l>r) { cout << "0 0\n"; continue; }
        auto L = lower_bound(all(arr), l);
        auto R = upper_bound(all(arr), r);
        i64 cnt = R-L;
        i64 sum = arr2[R-arr.begin()] - arr2[L-arr.begin()];
        println(cnt, ' ', sum);
    }
}
