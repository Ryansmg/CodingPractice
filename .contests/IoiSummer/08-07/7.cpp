...
void makeTree(i64 v) {
    for(const i64 &i : conRaw[v]) {
        if(vis[i] || fin[i]) continue;
        vis[i] = true;
        con[v].push_back(i);
        makeTree(i);
    }
}
void makeTree() { vis[1] = true; makeTree(1); }

i64 setSize(i64 v, i64 b = -1) {
    sz[v] = 1;
    for(const i64 &i : con[v])
        if(i != b && !fin[i]) sz[v] += setSize(i, v);
    return sz[v];
}

i64 getCent(i64 v, i64 b = -1) {
    for(const i64 &i : con[v])
        if(i != b && sz[i]*2 > n && !fin[i]) return getCent(i, v);
    return v;
}

struct solve {
    i64 m, V, M=-1;
    solve(i64 lim, i64 vertex) {
        m = lim;
        V = vertex;
        if(m>17) {
            cout << -1;
    ...
    void operator()(){
        vis = v<bool>(n+10, false);
        makeTree(V);
        setSize(V);
        vis = v<bool>(n+10, false);
        construct(getCent(V), m);
    }

    void construct(i64 v, i64 d = 1) {
        queue<ii> bfs;
        bfs.push({v, d});
        bool next = false;
        while(!bfs.empty()) {
            ...
            if(next && top[1]>M) {
                ...
                ans[top[0]] = top[1];
                fin[top[0]] = true;
                if(conRaw[top[0]].empty()) {
                    next = true;
                    M = top[1];
                    solve(M+1, top[0])();
                    continue;
                }
                for(const i64 &i : conRaw[top[0]]) {
                    if(vis[i] || fin[i]) continue;
                    vis[i] = true;
                    bfs.push({i, top[1]+1});
                }
...
i32 main() {
    fastio;
    cin >> n;
    conRaw = con = v2<i64>(n+10, v<i64>());
    vis = fin = v<bool>(n+10, false);
    sz = ans = v<i64>(n+10, 0);
    forn(i, n-1) {
        i64 a, b; cin >> a >> b;
