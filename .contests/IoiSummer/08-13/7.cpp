...
i32 main() {
    ...
    priority_queue<ii, v<ii>, greater<>> q;
    q.push({0, s});
    while(!q.empty()) {
        auto [dist, node] = q.top(); q.pop();
        if(vis2[node] <= dist) continue;
        if(vis[node] == dist) continue;
        if(vis[node] == INF) {
            vis[node] = dist;
            for(auto [nd, w] : con[node]) {
                if(vis2[nd] <= dist+w) continue;
                if(vis[nd] == dist+w) continue;
                q.push({dist+w, nd});
            }
            continue;
        }
