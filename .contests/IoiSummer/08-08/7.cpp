...
template <typename T> v<T> inputArr(i64 sz) { v<T> a(1, 0); forn(i,sz)a.push_back(input<T>()); return a; }
...
i32 main() {
    ...
    i64 n, m; cin >> n >> m;
    v2<i64> island, c;
    island.emplace_back(m+1, 0);
    forn(i, n) island.push_back(inputArr<i64>(m));
    c = v2<i64>(n+1, v<i64>(m+1, 0));
    forf(i, 1, n) {
        forf(j, 1, m) {
            if(island[i][j-1] != island[i][j]) c[i][j] = 1;
