...
struct tr {
    ...
    tr operator+(const tr& b) const {
        return {i32((i64(v)+i64(b.v))%mod9)};
    }
};


i64 usedMem = 0;
array<tr, 100000000> tree;
array<i32, 100000000> l, r;

template <typename T = long long>
class DynamicSeg {
    static inline long long m(long long s, long long e) { return s + (e - s) / 2; }
    long long ln, rn; i32 root = 0;
    signed next() { tree[usedMem] = tr(); l[usedMem] = r[usedMem] = -1; return usedMem++; }
public:
    explicit DynamicSeg(long long li, long long ri) : ln(li), rn(ri) { root = next(); }
    void add(long long tar, const T& val) { add(root, ln, rn, tar, val); }
    void set(long long tar, const T& val) { set(root, ln, rn, tar, val); }
    T query(long long left, long long right) { return query(root, ln, rn, left, right); }
private:
