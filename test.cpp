...
i32 main() {
    fastio;
    in64(n);
    map<ii, i64> pre; pre.insert({{0, 0}, 0});
    rep(n) {
        in64(x, y);
        map<ii, i64> cur; i64 a = INF;
        for(auto &[pos, ans] : pre) {
            ii xpos = {x, pos[1]}; i64 xans = ans + abs(x-pos[0]);
            auto xi = cur.find(xpos);
            if(xi == cur.end()) cur.insert({xpos, xans}), a = min(a, xans);
            else xi->second = min(xans, xi->second), a = min(a, xi->second);

            ii ypos = {pos[0], y}; i64 yans = ans + abs(y-pos[1]);
            auto yi = cur.find(ypos);
            if(yi == cur.end()) cur.insert({ypos, yans}), a = min(a, yans);
            else yi->second = min(yans, yi->second), a = min(a, yi->second);
        }
        pre = cur;
        while(a + 4000 < pre.rbegin()->second) pre.erase(prev(pre.end()));
    }
    i64 ans = INF;
    for(auto &[t, a] : pre) ans = min(ans, a);
    println(ans);
}
