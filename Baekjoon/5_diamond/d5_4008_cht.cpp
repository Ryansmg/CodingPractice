...
struct CHNLine {
    i64 x, y;
    CHNLine(const i64 x = 0, const i64 y = 0) : x(x), y(y) {}
...
// dp[i] : 1~i에서의 정답
// s[i] = sum(1, i)
// f(l, r) <- x:=sum(l..r), ax^2+bx+c
// 이때 bx 항은 항상 일정 -> 무시
// dp[i] = max(dp[j] + f(j+1, i)) *f(1, i) 고려
//       = max(dp[j] + a*sum(j+1, i)^2 + c)
//       =           + a * (s[i] - s[j])^2 + c
//                         (s[i]^2 - 2s[i]s[j] + s[j]^2)
//       = max((dp[j] + a*s[j]^2) - 2as[j] * s[i]) + a*s[i]^2 + c
...
i32 main() {
    ...
    in64(a, b, c);
    vl s = inArr(n);
    forf(i, 1, n-1) s[i] += s[i-1];
    forn(i, n) {
        i64 ans = s[i]*s[i]*a + c;
        if(i) ans = max(ans, lc.Query(s[i]) + s[i]*s[i]*a + c);
        if(i == n-1) println(ans + s.back() * b);
        lc.Insert(-2*a*s[i], ans + a*s[i]*s[i]);
    }
}
