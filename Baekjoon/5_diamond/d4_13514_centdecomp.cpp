...
v2l par;
vl depth;
v<multiset<i64>> ss; // dist
vb color;
i64 max_depth = 0, logmd = 0;
...
i64 dist(i64 a, i64 b) {
    ...
    for(i64 i = logmd; depDiff && i >= 0; i--)
        if(depDiff & (1<<i)) b = par[b][i], depDiff -= (1<<i), ans += (1<<i);

    for(i64 i = logmd; i >= 0; i--)
        if(par[a][i] != par[b][i])
            a = par[a][i], b = par[b][i], ans += (1 << (i + 1));
...
void update(i64 i) {
    ...
    color[i] = !color[i];
    while(true) {
        if(color[i]) ss[j].emplace(dist(i, j));
        else ss[j].erase(ss[j].find(dist(i, j)));
        if(ct_par[j] == j) break;
        j = ct_par[j];
    }
}
...
i64 query(i64 i) {
    ...
    i64 j = i;
    while(true) {
        if(!ss[j].empty()) ans = min(ans, *ss[j].begin() + dist(i, j));
        if(ct_par[j] == j) break;
        j = ct_par[j];
    }

    return replace_if(ans, inf, -1LL);
}

void init_par_0(i64 node = 1, i64 p = 1, i64 dep = 1) {
    par[node].eb(p); depth[node] = dep;
    max_depth = max(max_depth, dep);
    for(ci64 i : adj[node]) {
        if(i == p) continue;
        init_par_0(i, node, dep+1);
    }
}

void init_par_1() {
    logmd = log2(max_depth) + 1;
    forn(i, logmd) {
        forf(j, 1, n) par[j].eb(par[par[j][i]][i]);
    }
}

void calc_size(i64 node, i64 par = -1) {
    sz[node] = 1;
    for(ci64 i : adj[node]) {
...
i64 get_centroid(i64 node, i64 size = -1, i64 par = -1) {
    if(size == -1) {
        calc_size(node);
        size = sz[node];
    }
    for(ci64 i : adj[node]) {
        if(vis[i] || par == i) continue;
        if(sz[i] * 2 > size) return get_centroid(i, size, node);
    }
    vis[node] = true; return node;
}

i64 init_ct_tree(i64 node) {
    i64 cent = get_centroid(node);
    ct_par[cent] = cent;
    for(ci64 i : adj[cent]) {
        if(vis[i]) continue;
        i64 ch = init_ct_tree(i);
        ct_child[cent].eb(ch);
        ct_par[ch] = cent;
    }
    return cent;
