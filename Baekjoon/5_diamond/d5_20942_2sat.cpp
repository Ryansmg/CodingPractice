...
i32 main() {
    ...
    TwoSat ts(5*n);
    forn(i, n) {
        ts.addXor(5*i+4+1, 5*i+3+1), ts.addOr(-(5*i+4+1), -(5*i+2+1));
        if(a[i]) forn(j, 5) {
            if(a[i] & (1<<j)) ts.addTrue(5*i+j+1);
            else ts.addFalse(5*i+j+1);
        }
    }
    in64(m);
    rep(m) {
        char c = input<char>(); in64(x, y, z); x--; y--;
        if(c == '&') {
            if(a[x] && a[y]) { if((a[x]&a[y])!=z) printExit(0); }
            else {
                if(!a[x] && !a[y]) {
                    forn(i, 5) {
                        if(z&(1<<i)) ts.add(x*5+i+1), ts.add(y*5+i+1);
                        else ts.add(-(x*5+i+1), -(y*5+i+1));
                    }
                } else if(a[x]) {
                    forn(i, 5) {
                        if((z&(1<<i)) && (a[x]&(1<<i))) ts.add(y*5+i+1);
                        elif(z&(1<<i)) printExit(0);
                        elif(a[x]&(1<<i)) ts.addFalse(y*5+i+1);
                    }
                } else if(a[y]) {
                    forn(i, 5) {
                        if((z&(1<<i)) && (a[y]&(1<<i))) ts.add(x*5+i+1);
                        elif(z&(1<<i)) printExit(0);
                        elif(a[y]&(1<<i)) ts.addFalse(x*5+i+1);
                    }
                }
            }
        } else {
            if(a[x] && a[y]) { if((a[x]|a[y])!=z) printExit(0); }
                ...
                if(!a[x] && !a[y]) {
                    forn(i, 5) {
                        if(z&(1<<i)) ts.add(x*5+i+1,y*5+i+1);
                        else ts.add(-(x*5+i+1)), ts.add(-(y*5+i+1));
                    }
                } else if(a[x]) {
                    forn(i, 5) {
                        if((z&(1<<i)) && (a[x]&(1<<i))) { }
                        elif(z&(1<<i)) ts.add(y*5+i+1);
                        elif(a[x]&(1<<i)) printExit(0);
                        else ts.addFalse(y*5+i+1);
                    }
                } else if(a[y]) {
                    forn(i, 5) {
                        if((z&(1<<i)) && (a[y]&(1<<i))) { }
                        elif(z&(1<<i)) ts.add(x*5+i+1);
                        elif(a[y]&(1<<i)) printExit(0);
                        else ts.addFalse(x*5+i+1);
                    }
                }
            }
        }
    ...
    vb ans = ts.getAns();
    if(ans.empty()) printExit(0);
    println(1);
    i64 cur = 0;
    forf(i, 1, 5*n) {
        if(ans[i]) cur ^= (1 << ((i-1)%5));
        if(i%5==0) printf(.end=" ")(cur), cur = 0;
    }
}
