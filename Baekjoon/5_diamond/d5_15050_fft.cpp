...
i32 main() {
    ...
    str s = in<str>();
    vl i;
    for(const char &c : s) i.eb(c-'a'+1);
    i64 mx = i[0]; i64 sz = s.length();
    forf(j, 1, sz-1) mx = max(mx, i[j] += i[j-1]);
    vl a(mx+10, 0), b(mx+mx+10, 0);
    a[mx]++; for(cint k : i) a[mx-k]++, b[k+mx]++;
    vl c = a * b;
    i64 ans = 0;
    forf(j, mx*2+1, c.size()-1) if(c[j]) ans++;
    println(ans);
}
