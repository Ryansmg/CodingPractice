...
i32 main() {
    ...
    v<pst> psts(1, pst());
    psts[0].init();
    i64 type, tar, l, r, x, k;
    inRep() {
        in(type);
        if(type == 1) {
            pst p = pst::newFrom(psts.back());
            in(tar);
            p.add(psts.back(), tar, 1);
            psts.emplace_back(p);
        }
        else if(type == 2) {
            in(l, r, x);
            // A의 L번째 수부터 R번째 수까지 중에서 x와 xor한 값이 가장 큰 y를 찾아 출력한다.
            i64 prvm = 0, prvM = 0b1111111111111111111;
            auto cnt = [&](i64 m, i64 M) {
                return psts[r].query(m, M) - psts[l-1].query(m, M);
            };
            forr(i, 18, 0) {
                i64 cur = (1LL<<i) ^ ((1LL<<i)&x);
                if(cur) {
                    if(cnt(prvm ^ cur, prvM)) prvm ^= cur;
                    else prvM ^= cur;
                } else {
                    i64 t = 1LL<<i;
                    if(cnt(prvm, prvM^t)) prvM ^= t;
                    else prvm ^= t;
                }
            }
            assert(prvm == prvM);
            ...
            // A의 L번째 수부터 R번째 수까지 중에서 x보다 작거나 같은 원소의 개수를 출력한다.
            in(l, r, x);
            println(psts[r].query(1, x) - psts[l-1].query(1, x));
        } else if(type == 5) {
            // A의 L번째 수부터 R번째 수까지 중에서 k번째로 작은 수를 출력한다.
            in(l, r, k);
            ...
            println(q(psts[l-1].pos, psts[r].pos, 1, PST::n, k));
        }
    }
}
