...
i32 main() {
    ...
    tcRep() {
        in64(n);
        Graph<DistEdge> g(n);
        rep(n-1) g.makeUEdge(qin(3));
        Tree t(1, g); g.clear();
        pst tr(0, 100000);
        v<pstRoot> psts(n+1);
        psts[0] = psts[1] = tr.begin();
        fun<void(i64)> initPst = [&](i64 node) {
            for(auto& [s, e, d] : t.child[node]) {
                psts[e] = psts[s].next().add(d, 1);
                initPst(e);
            }
        };
        initPst(1);
        auto getKth = [&](pstIter p1, pstIter p2, pstIter m, i64 k) {
            while(p1.s != p1.e) {
                auto lp1 = p1.left(), lp2 = p2.left(), lm = m.left();
                i64 cnt = *lp1 + *lp2 - *lm * 2;
                if(k <= cnt) {
                    p1 = lp1; p2 = lp2; m = lm;
                } else {
                    k -= cnt;
                    p1 = p1.right(); p2 = p2.right(); m = m.right();
                }
            }
            return p1.s;
        };
        ...
        inRep() {
            in64(a, b);
            i64 lca = t.lca(a, b);
            auto p1 = psts[a].getIter(), p2 = psts[b].getIter(), m = psts[lca].getIter();
            i64 edgeCnt = *p1 + *p2 - *m * 2;
            if(edgeCnt&1) println(getKth(p1, p2, m, edgeCnt/2+1), ".0");
            else {
                i64 s = getKth(p1, p2, m, edgeCnt/2) + getKth(p1, p2, m, edgeCnt/2+1);
                println(s/2, s&1 ? ".5" : ".0");
            }
        }
    }
