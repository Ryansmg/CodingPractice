...
public:
    struct Node {
        ...
        inline void forEach(const Callable& f) { Splay<T>::forEach(this, f); }
        template <typename Callable>
        inline void preorder(const Callable& f) { Splay<T>::preorder(this, f); }
    protected:
        inline void upd() {
            if constexpr (hasUpdFunc<T>) {
                if(l) l->push();
                if(r) r->push();
                v.update(l ? &l->v : nullptr, r ? &r->v : nullptr);
            }
            if(sp->updFun) {
                if(l) l->push();
                if(r) r->push();
        ...
        inline void push() {
            if constexpr(hasPushFunc<T>) {
                v.push(l ? &l->v : nullptr, r ? &r->v : nullptr);
            }
...
public:
    ...
    Splay() = default;
    explicit Splay(int length) : sz(length) {
        if(!length) return;
        ...
        while(x->p) x = x->p, x->upd();
    }
    explicit Splay(const std::vector<T>& arr) {
...
void update(Splay<GoldMine>::Node& a) {
    a.v.mx = a.v.l = a.v.r = max(0, a.v.v);
    a.v.s = a.v.v;
...
int main() {
    int n = get();
    vec<GoldMine> arr(n + 2);
    for(int i = 1; i <= n; i++) {
        if(get()) {
            arr[i] = {1, 1, 1, 1, 1};
        } else {
            arr[i] = {-15571557, -15571557, -15571557, -15571557, -15571557};
        }
        arr[i].id = i;
    }
    Splay<GoldMine> splay(arr);
    splay.setPushFun(push);
    splay.setUpdFun(update);

    int m = get();
    while(m--) {
        long long op, a, b; get(op, a, b);
        if(op == 1) {
            auto x = splay.gather(a, b);
            auto v = x->v;
            splay.set(x, {v.v, v.mx, v.l, v.r, v.s, v.id, !v.flip});
        }
        else ln(splay(a, b).mx);
    }
}
