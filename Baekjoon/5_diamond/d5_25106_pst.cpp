...
Tpl64 struct Fenwick {
    ...
    void update(i32 tar, const T& val) { if(tar <= 0) { return; } for(; tar<=n; tar+=tar&-tar) tree[tar] += val; }
    /// [l, r]
...
i32 main() {
    fastio;
    in64(n, m, k);
    pst<GoldMine> tr(v<GoldMine>(m, 0));
    v<pst<GoldMine>::root> psts(1, tr.begin());
    v2<ii> fish(n+1, v<ii>());
    vl cv(m+1, 0);
    rep(k) {
        in64(r, c, v);
    ...
    forf(i, 1, n) {
        auto cur = psts.back().next();
        for(const auto &[c, v] : fish[i]) cur.set(c, cv[c] + v), cv[c]+=v;
        psts.eb(cur);
    }
    i64 a1 = 0, a2 = 0, a3 = 0;
    inRep() {
        ...
        h ^= a1, x ^= a2, y ^= a3;
        a1 = a2, a2 = a3;
        println(a3 = max(0, psts[h].query(x, y).mx));
    }
}
