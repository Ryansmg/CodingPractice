...
{
    fastio;
    int n = input(), sqrtSize = 1000,
    bucketCnt = (n / sqrtSize) + 10;
    ...
    for(int qi=0; qi<m; qi++) {
        ...
        if(type == update) {
            ...
            buckets[bucketInd].erase(lower_bound(all(buckets[bucketInd]), preVal));
            buckets[bucketInd].insert(upper_bound(all(buckets[bucketInd]), val), val);
        }
    }
}
