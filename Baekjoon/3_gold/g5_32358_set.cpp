...
i32 main() {
    set<i64> arr; i64 pos = 0, ans = 0;
    i64 n = in();
    rep(n) {
        i64 t = in();
        if(t == 1) arr.insert(in());
        else {
            while(!arr.empty()) {
                auto iter = arr.upper_bound(pos);
                auto iter2 = iter;
                iter2--;
                i64 leftDist = INF, rightDist = INF;
                if(iter != arr.end()) rightDist = *iter - pos;
                if(iter != arr.begin()) leftDist = pos - *(iter2);
