...
struct nd {
    i64 mx = 0, mn = 0;
    nd()=default;
    nd(i64 mx_, i64 mn_) : mx(mx_), mn(mn_) {}
};

v<nd> tree; vl lazy;
i64 c, n, o;

void push(i64 s, i64 e, i64 p) {
    tree[p].mx += lazy[p];
    tree[p].mn += lazy[p];
    if(s != e) lazy[p*2] += lazy[p], lazy[p*2+1] += lazy[p];
    lazy[p] = 0;
}
...
void update(i64 l, i64 r, i64 x, i64 s = 1, i64 e = c, i64 p = 1) {
    push(s, e, p);
    if(e < l || r < s) return;
    if(l <= s && e <= r) {
        lazy[p] += x;
        ...
        i64 m = (s + e) >> 1;
        update(l, r, x, s, m, p*2);
        update(l, r, x, m+1, e, p*2+1);
        tree[p] = merge(tree[p*2], tree[p*2+1]);
    }
}

nd query(i64 l, i64 r, i64 s = 1, i64 e = c, i64 p = 1) {
    push(s, e, p);
    if(e < l || r < s) return {-INF, INF};
    if(l <= s && e <= r) return tree[p];
    i64 m = (s + e) >> 1;
    return merge(query(l, r, s, m, p*2), query(l, r, m+1, e, p*2+1));
}
...
i32 main() {
    ...
    rep(o) {
        ...
        if(t[0] == 's') {
            i64 x = input() + 1;
            println(query(x, x).mx);
        } else if(t[0] == 'g') {
            invar(a, b, s); a++; b++;
            nd cur = query(a, b);
            if(s > 0) s = min(s, n - cur.mx);
            if(s < 0) s = max(s, -cur.mn);
            println(s);
            update(a, b, s);
        } else {
            invar(x, s); x++;
