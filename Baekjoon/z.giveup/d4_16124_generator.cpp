...
void println() { cout << '\n'; }
template <typename T> void printSep(const T& a_) { cout << a_; }
...
i32 main() {
    ...
    invar(len, q);
    str s(len, '0');
    for(char &c : s) c = rand() % 10 + '0';
    println(s);
    println(q);
    rep(q) {
        if(rand() & 1) {
            i64 l = randInt(1, len), r = randInt(1, len);
            if(l > r) swap(l, r);
            printSepLn(1, l, r, randInt(0, 9), randInt(0, 9));
        } else {
            i64 l = randInt(1, len), r = randInt(1, len);
            if(l > r) swap(l, r);
            printSepLn(2, l, r);
