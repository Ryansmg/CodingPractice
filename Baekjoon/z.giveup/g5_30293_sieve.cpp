...
i32 main() {
    ...
    forf(i, 2, n) {
        if(pcnt[i]) continue;
        pcnt[i] = 1;
        for(i64 r = i + i; r <= n; r += i) {
            i64 t = r;
            while(t % i == 0) t/=i, pcnt[r]++;
        }
    }
    
    vl arr; forf(i, 1, n) arr.eb(i);
    sort(all(arr), [&](const i64& a, const i64& b) -> bool {
        if(pcnt[a] == pcnt[b]) return a < b;
        return pcnt[a] < pcnt[b];
    });
    vl t = inArr(n);
    i64 ans = 0;
    for(ci64 i : arr) {
        ans += abs(t[i-1]);
        for(i64 j = i + i; j <= n; j += i) t[j-1] -= t[i-1];
        t[i-1] = 0;
    }
    println(ans);
}
