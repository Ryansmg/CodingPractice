...
public:
    vi tree; i32 n=-1;
    iterSeg() = default;
    explicit iterSeg(cint i) { tree = vi(i*2+10, 0); n = i; }
    /// 1 <= tar <= n
    void update(i32 tar, i32 val) {
        tar--; tree[n+tar] += val;
        for(i32 i = n+tar; i>1; i>>=1) tree[i>>1] = tree[i] + tree[i^1];
    }
    /// [l, r]
    i32 query(i32 left, i32 right) {
        if(left > right) return 0; left--;
        i32 l = n+left, r = n+right, ans = 0;
        for(; l<r; l>>=1, r>>=1) {
            if(l&1) ans += tree[l++];
            if(r&1) ans += tree[--r];
        }
...
struct query {
    i32 i, j, order;
    query(i32 a, i32 b, i32 c) {
        i = a; j = b; order = c;
    }
};

i32 sqrtsize; //sqrt(쿼리의 개수)
...
i32 main() {
    ...
    invar(n, m); i32 a, b;
    vl ans(m);
    vi arr(1, 1); rep(n) arr.eb(input());
    arr[0] = arr[1];
    vi comp = compressed_copy(arr);
    for(i32 &i : arr) i = idx(i, comp) + 1;
    
    iterSeg seg(n + 10);
    
    sqrtsize = sqrt(m);
    v<query> queries;
    forn(i, m) input(a, b), queries.eb(a, b, i);
    sort(all(queries), cmp);
    
    i64 cur = 0;
    i64 l = queries[0].i, r = queries[0].j;
    for(i32 i=l; i<=r; i++) {
        cur += seg.query(arr[i]+1, n+5);
        seg.update(arr[i], 1);
    }
    ...
    for(i64 i=1; i<m; i++) {
        ...
        while(nl < l) {
            l--;
            cur += seg.query(1, arr[l]-1);
            seg.update(arr[l], 1);
        }
        while(r < nr) {
            r++;
            cur += seg.query(arr[r]+1, n+5);
            seg.update(arr[r], 1);
        }
        while(l < nl) {
            cur -= seg.query(1, arr[l]-1);
