...
template <typename Type, typename Cmp = less<>> using pbds = __gnu_pbds::tree<Type, __gnu_pbds::null_type, Cmp, __gnu_pbds::rb_tree_tag, __gnu_pbds::tree_order_statistics_node_update>;
#endif
...
i32 main() {
    ...
    vl arr = inArr(n);
    vl l(n, 0), r(n, 0);
    pbds<i64, greater<>> set;
    forn(i, n) {
        set.insert(arr[i]);
        l[i] = set.order_of_key(arr[i]);
    }
    set = pbds<i64, greater<>>();
    forr(i, n-1, 0) {
        set.insert(arr[i]);
        r[i] = set.order_of_key(arr[i]);
    }
    i64 ans = 0;
    forn(i, n) {
