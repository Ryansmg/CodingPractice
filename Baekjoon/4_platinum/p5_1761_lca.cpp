...
v2<ii> adj; // tar, dist
vl depth;
v2<ii> par; // par[tar][log2(cnt)] == node, dist

i64 n, h;

void initTree(i64 node = 1, i64 pre = -1, i64 dep = 1) {
    depth[node] = dep;
    for(auto i : adj[node]) {
        if(i[0] == pre) continue;
        par[i[0]][0] = {node, i[1]};
        initTree(i[0], node, dep+1);
    }
}
...
void makePar() {
    ...
    forn(i, h) {
        forf(j, 1, n) {
            ii f = par[j][i];
            ii s = par[f[0]][i];
            par[j][i+1] = {s[0], f[1] + s[1]};
        }
    }
}

i64 findPar(i64 a, i64 b) {
    i64 ans = 0;
    if(depth[a] < depth[b]) swap(a, b);
    i64 depDiff = depth[a] - depth[b];
    forr(i, h-1, 0) {
        if(depDiff & (1LL<<i)) {
            ans += par[a][i][1];
            a = par[a][i][0];
    ...
    assert(a != b && par[a][0][0] == par[b][0][0]);
    return ans + par[a][0][1] + par[b][0][1];
}
