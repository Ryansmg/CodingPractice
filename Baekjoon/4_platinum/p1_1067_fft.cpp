...
namespace FFT {
    ...
    void fft(v<cpl> &a, bool inv) {
        ...
        // w 전처리
        v<cpl> roots;
        f128 w = 2 * acos(f128(-1)) / n * (inv ? -1 : 1);
        forn(i, n/2)
            roots.emplace_back(cos(w*i), sin(w*i));
        ...
        for(i64 i=2; i<=n; i<<=1) {
            ...
            for(j=0; j<n; j+=i) {
                forn(k, i/2) {
                    cpl u = a[j+k], v = a[j+k+i/2] * roots[step * k];
                    a[j+k] = u+v;
                    a[j+k+i/2] = u-v;
                }
            }
    ...
    template <typename T>
    v<T> operator*(v<T> &a, v<T> &b) {
        v<cpl> fa(all(a)), fb(all(b));
        i64 n = 2, sz = a.size() + b.size();
        while(n < sz) n <<= 1;
        ...
        fft(fa, true);
        v<T> ret;
        forn(i, n) ret.emplace_back((T)llround(fa[i].real()));
        return ret;
    }
}
...
i32 main() {
    i64 n = input();
    vl a = inputArr(n), b = inputArr(n);
    reverse(all(b));
    forn(i, n) a.emplace_back(a[i]);
    using namespace FFT;
    vl c = a * b;
    i64 ans = 0;
    forf(i, n-1, c.size()-1)
