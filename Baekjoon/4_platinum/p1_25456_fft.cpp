...
i32 main() {
    fastio;
    str s1, s2; input(s1, s2);
    vl a, b;
    for(const char& c : s1) a.emplace_back(c=='1');
    for(const char& c : s2) b.emplace_back(c=='1');
    reverse(all(b));
    vl ans = a * b;
    i64 r = 0;
    forn(i, b.size()-1)
        r = max(r, ans[i] + ans[ans.size()-b.size()+i+1]);
    for(cint i : ans) r = max(r, i);
    println(r);
}
