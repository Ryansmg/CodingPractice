...
#include <vector>
//true일 때 답 개수 1개
#define HAS_MULTIPLE_ANSWERS false
using namespace std;

long long processedbingos = 0;
...
void solve(vector<vector<bool>> &b, int recurdepth, int selectedcnt) {
    if(recurdepth != 25) {
        ...
        processedbingos++;
        //if (processedbingos % (65536*2) == 0) cout << processedbingos << " / 33554432\n";
        ...
        if(b[4][1]) if(!HAS_MULTIPLE_ANSWERS) return;
        if(!b[4][1] && HAS_MULTIPLE_ANSWERS) return;
        if(b[4][3]) {
