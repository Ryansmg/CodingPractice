...
public:
    v<int> tree; int n=-1;
    iterSeg() = default;
    explicit iterSeg(cint i) { tree = v<int>(i*2+10, INF); n = i; }
    /// 1 <= tar <= n
    void update(int tar, int val) {
        assert(1 <= tar && tar <= n);
        tar--;
        tree[n+tar] = val;
        for(int i = n+tar; i>1; i>>=1) tree[i>>1] = min(tree[i], tree[i^1]);
    }
    /// [l, r]
    int query(int left, int right) {
        assert(1 <= left && right <= n);
        left--;
        int l = n+left, r = n+right, ans = INF;
        for(; l<r; l>>=1, r>>=1) {
...
private:
    void init() { for(int i=n-1; i>0; i--) tree[i] = min(tree[i<<1], tree[i<<1|1]); }
};

v<i64> sz, dep, par, top, in, out, vis, ori;
v2<i64> con, conraw;
i64 pv = 0;
...
i64 query(i64 a, i64 b) {
    ...
    while(top[a] != top[b]) {
        ...
        i64 st = top[a];
        ret = min(ret, seg.query(in[st], in[a]));
        a = par[st];
    }
    if(dep[a] > dep[b]) swap(a, b);
    ret = min(ret, seg.query(in[a], in[b]));
    return ret;
}
...
i32 main() {
    fastio;
    invar(n);
    sz = dep = par = top = in = out = vis = ori = vl(n+1, 0);
    con = conraw = v2l(n+1, vl());
    rep(n-1) {
        invar(u, v);
        conraw[u].eb(v);
        conraw[v].eb(u);
    ...
    inRep() {
        invar(t, v);
        if(t == 1) {
            if(seg.query(in[v], in[v]) == INF) seg.update(in[v], in[v]);
            else seg.update(in[v], INF);
        } else {
            i64 ans = query(1, v);
            if(ans == INF) println(-1);
            else println(ori[ans]);
        }
    }
}
