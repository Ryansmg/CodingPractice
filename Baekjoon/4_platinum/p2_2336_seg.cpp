...
public:
    ...
    int query(int tar) { return tree[tar+n]; }
    /// [l, r]
...
i32 main() {
    ...
    i32 n = input();
    vl x(n), y(n), z(n);
    forn(i, n) x[input()-1] = i;
    forn(i, n) y[input()-1] = i;
    forn(i, n) z[input()-1] = i;
    v<iii> arr;
    forn(i, n) arr.push_back({x[i], y[i], z[i]});
    sort(arr);
    iterSeg seg(n);
    i64 ans = n;
    forn(i, n) {
        i64 q = seg.query(0, arr[i][1]);
        if(q < arr[i][2]) ans--;
        seg.update(arr[i][1], arr[i][2]);
    }
    println(ans);
}
