...
struct lz {
    i32 v = -1;
    lz operator+(ci32 i) const { return {i}; }
};
lz operator+(const lz& a, SparseSeg<i32, lz, i32>::iter&& b) {
    if(b.lazy.v == -1) return a;
    return {b.lazy.v};
}
i32 operator+(ci32 a, SparseSeg<i32, lz, i32>::iter&& b) {
    if(b.lazy.v == -1) return a;
    return (b.end - b.start + 1) * b.lazy.v;
}

i32 main() {
    fastio;
    in64(n);
    SparseSeg<i32, lz, i32> seg(0, 1000000000);
    i32 c = 0;
    rep(n) {
        if(qin() == 1) {
            in64(a, b);
            println(c = seg.query(a+c, b+c));
        }
        else {
            in64(a, b);
            seg.update(a+c, b+c, 1);
        }
    }
}
