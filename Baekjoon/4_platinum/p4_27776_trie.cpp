...
struct trie {
    trie* child[26]{};
    i32 val = 0;
    trie() { for(auto &i : child) i = nullptr; }
    ~trie() { for(auto &p : child) delete p; }
    void add(str& s) {
        i64 nidx = s.back() - 'A';
        s.pop_back();
        if(!child[nidx]) child[nidx] = new trie();
        child[nidx]->val++;
        if(!s.empty()) child[nidx]->add(s);
    }
    i64 query() {
        i64 ans = 0;
        if(val < 2) return 0;
...
i32 main() {
    ...
    tcRep() {
        ...
        i64 ans = 0;
        for(auto &p : t.child) if(p) ans += p->query();
        println(ans);
    }
}
