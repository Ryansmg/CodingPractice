...
i64 inRep_var_;
#define inRep input(inRep_var_); forn(inRep_var_2_, inRep_var_)
...
i32 main() {
    fastio;
    vl u(60010), i(60010), m;
    inRep u[input()+30000]++;
    inRep m.emplace_back(input()+30000);
    inRep i[input()+30000]++;
    vl ans = u * i; i64 ret = 0;
    for(cint k : m) ret += ans[k*2];
    println(ret);
}
