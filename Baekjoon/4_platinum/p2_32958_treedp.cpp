...
struct t {
    i64 downSum, downWeight, upSum, upCnt, ans;
    void applyMod() {
        downSum %= mod1;
        downWeight %= mod1;
        upSum %= mod1;
...
t solve(i64 node, i64 par = -1) {
    t ans = {s[node], 10, s[node], 1, s[node]};
    t cans = ans;
    lprintln("solve for ", node);
    for(ci64 i : adj[node]) {
        if(i == par) continue;
        t r = solve(i, node);
        ans.downSum += r.downSum + r.downWeight * s[node];
        ans.downWeight += r.downWeight * 10;
        ans.upSum += r.upSum * 10 + s[node] * r.upCnt;
        ans.upCnt += r.upCnt;
        i64 tt = 0;
        tt += r.downSum * cans.upCnt + r.downWeight * cans.upSum;
        tt += r.upSum * cans.downWeight + cans.downSum * r.upCnt;
        ans.ans += tt;
        lprintvar(cans.downSum, cans.downWeight, cans.upSum);
        lprintvar(tt, r.ans); ans.ans += r.ans;
        cans.downWeight += r.downWeight * 10;
        cans.upSum += r.upSum * 10 + s[node] * r.upCnt;
        cans.downSum += r.downSum + r.downWeight * s[node];
        cans.upCnt += r.upCnt;
        ans.applyMod(); cans.applyMod();
    }
    lprintln("end ", node);
    return ans;
}
...
i32 main() {
    ...
    adj.resize(v+1, vl());
    inArr(s, v);
    rep(v-1) { in64(a, b); adj[a].eb(b); adj[b].eb(a); }
    println(solve(1).ans);
}
