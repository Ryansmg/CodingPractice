...
i32 main() {
    ...
    v<GLine<i64>> lines;
    forn(i, n-1) lines.eb(arr[i][0], arr[i][1], arr[i+1][0], arr[i+1][1]);
    lines.eb(arr[n-1][0], arr[n-1][1], arr[0][0], arr[0][1]);
    rep(3) {
        i64 is = 0;
        in64(x, y);
        GLine il(x, y, inf, y);
        for(const auto& edge : lines) {
            if(edge.contains({x, y})) { println(1); goto end; }
            if(edge.p1.y == y || edge.p2.y == y) {if(edge.intersects(il)) is += min(edge.p1.y, edge.p2.y) < y;}
            else is += edge.intersects(il);
        }
        println(is&1);
        end:{}
    }
