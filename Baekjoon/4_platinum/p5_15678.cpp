...
public:
    ...
    void update(int tar, int val) {
        assert(0 <= tar && tar < n);
        tree[n+tar] = val;
        for(int i = n+tar; i>1; i>>=1) tree[i>>1] = max(tree[i], tree[i^1]);
    }
    /// [l, r]
...
private:
    void init() { for(int i=n-1; i>0; i--) tree[i] = max(tree[i<<1], tree[i<<1|1]); }
...
i32 main() {
    ...
    i64 n, d; input(n, d);
    iterSeg seg(n);
    auto arr = inputArr<i64>(n);
    i64 ans = -INF;
    forn(i, n) {
        i64 M = seg.query(max(0LL, i-d), i);
        ans = max(ans, M+arr[i]);
        seg.update(i, M+arr[i]);
    }
    println(ans);
}
