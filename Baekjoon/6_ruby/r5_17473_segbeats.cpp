...
void lprintvar_(const string& names_, Args... args) {
    ...
    auto print_each = [&](auto&& value) {
        ...
        cout << name << ": " << value << (pos == string::npos ? "\n" : ", ");
...
/// requirements: (TreeType + TreeType), (LazyType + UpdateType), (TreeType + LazyIter&&), (LazyType + LazyIter&&)
/// <br> usage: node merge, node update, lazy update, lazy update
template <typename TreeType = tr, typename LazyType = lz, typename UpdateType = lz>
class lp {
public:
    struct iter {
        i32 node, start, end; TreeType tree; LazyType lazy; lp* segPtr;
    };
protected:
    v<TreeType> tree; v<LazyType> lazy; i32 n=-1;
    void push(i32 node, i32 start, i32 end) {
...
public:
    explicit lp(i32 treeSize) { tree = v<TreeType>(4*treeSize, TreeType()); lazy = v<LazyType>(4*treeSize, LazyType()); n = treeSize; }
    explicit lp(const v<TreeType> &a) : lp((i32) a.size()) { init(a, 1, 1, n); }
    void update(i32 left, i32 right, UpdateType diff) { update(1, left, right, 1, n, diff); }
    TreeType query(i32 left, i32 right) { return query(1, left, right, 1, n); }
    TreeType query(i32 tar) { return query(tar, tar); }
    iter root() { push(1, 1, n); return iter(1, 1, n, tree[1], lazy[1], this); }
protected:
    ...
    TreeType update(i32 node, i32 left, i32 right, i32 start, i32 end, UpdateType diff) {
        ...
        if(diff.orv == 0) { // and 연산
            if((tree[node].orv & (~diff.andv)) == 0) return tree[node];
        } else {
            if((tree[node].andv & diff.orv) == diff.orv) return tree[node];
        }
        bool tag = tree[node].andv == tree[node].orv;
        if(diff.orv == 0) {
            if((tree[node].andv & (~diff.andv)) == (tree[node].orv & (~diff.andv))) tag = true;
        } else {
            if((tree[node].orv & diff.orv) == (tree[node].andv & diff.orv)) tag = true;
        }
        if(left <= start && end <= right && tag) {
...
struct tr {
    i64 maxv = 0, andv = (1 << 25) - 1, orv = 0;
    tr() = default;
    explicit tr(i64 v) : maxv(v), andv(v), orv(v) {}
    tr(i64 a, i64 b, i64 c) : maxv(a), andv(b), orv(c) {}
    tr operator+(const tr& b) const { return {max(maxv, b.maxv), andv & b.andv, orv | b.orv}; }
};

struct lz {
    i64 andv = (1 << 25) - 1, orv = 0;
    lz operator+(const lz& i) const { return {i.andv, i.orv}; }
};

tr operator+(const tr& a, const lp<tr, lz, lz>::iter& b) {
    return { (a.maxv | b.lazy.orv) & b.lazy.andv, (a.andv | b.lazy.orv) & b.lazy.andv, (a.orv | b.lazy.orv) & b.lazy.andv };
}

lz operator+(const lz& a, const lp<tr, lz, lz>::iter& b) {
    return { (a.andv | b.lazy.orv) & b.lazy.andv, (a.orv | b.lazy.orv) & b.lazy.andv };
}


i32 main() {
    fastio;// filein; ansout;
    // in64(n);
    // vl arr = inArr(n);
    // inRep() {
    //     in64(t, l, r);
    ...
    //     else {
    //         i64 mx = 0;
    //         forf(i, l, r) mx = max(mx, arr[i-1]);
    //         println(mx);
    //     }
    // }

    in64(n);
    v<tr> arr = castVec<tr>(inArr<i64>(n));
    lp<tr, lz, lz> lp(arr);
    inRep() {
        in64(t, l, r);
        if(t == 1) lp.update(l, r, {input(), 0});
        elif(t == 2) lp.update(l, r, {(1<<25)-1, input()});
        else println(lp.query(l, r).maxv);
    }

    // i64 n = randInt(1, 20);
    // println(n);
    // rep(n) print(randInt(0, 1000), " ");
    // println();
    // i64 m = randInt(10, 30);
    // println(m);
    // rep(m) {
    //     i64 t = randInt(1, 3);
    //     i64 l = randInt(1, n), r = randInt(1, n);
    //     if(l > r) swap(l, r);
    //     if(t < 3) {
    //         printfln()(t, l, r, randInt(0, 2048));
    //     } else {
    //         printfln()(t, l, r);
    //     }
    // }
