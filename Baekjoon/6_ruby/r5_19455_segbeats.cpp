...
struct tr {
    i64 minv = INF, andv = (1LL << 32) - 1, orv = 0;
    tr() = default;
    explicit tr(i64 v) : minv(v), andv(v), orv(v) {}
    tr(i64 a, i64 b, i64 c) : minv(a), andv(b), orv(c) {}
    tr operator+(const tr& b) const { return {min(minv, b.minv), andv & b.andv, orv | b.orv}; }
};

struct lz {
    i64 andv = (1LL << 32) - 1, orv = 0;
    lz operator+(const lz& i) const { return {i.andv, i.orv}; }
};

tr operator+(const tr& a, const lp<tr, lz, lz>::iter& b) {
    return { (a.minv | b.lazy.orv) & b.lazy.andv, (a.andv | b.lazy.orv) & b.lazy.andv, (a.orv | b.lazy.orv) & b.lazy.andv };
}
...
i32 main() {
    ...
    inRep() {
        char t = input<char>();
        in64(l, r);
        if(t == '&') lp.update(l, r, {input(), 0});
        elif(t == '|') lp.update(l, r, {(1LL<<32)-1, input()});
        else println(lp.query(l, r).minv);
    }
}
