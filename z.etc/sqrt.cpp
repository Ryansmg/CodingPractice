...
inline BigInt gcd_(BigInt a, BigInt b) { if(a<b) swap(a, b); while(!b.isZero()) { BigInt r = a % b; a = b; b = r; } return a; }

struct frac {
    BigInt numerator;
    BigInt denominator;
    frac(BigInt a, BigInt b) : numerator(std::move(a)), denominator(std::move(b)) {}
    frac operator*(int o) const { return *this * frac(o, 1); }
    frac operator+(frac o) const {
        frac t = *this;
        BigInt dg = gcd_(denominator, o.denominator);
        o.numerator *= t.denominator / dg;
        t.denominator /= dg; t.denominator *= o.denominator;
        t.numerator *= o.denominator / dg;
        t.numerator += o.numerator;
        dg = gcd_(t.numerator, t.denominator);
        t.numerator /= dg; t.denominator /= dg;
        return t;
    }
    frac operator*(const frac &o) const {
        frac t = *this;
        t.numerator *= o.numerator;
        t.denominator *= o.denominator;
        BigInt dg = gcd_(t.numerator, t.denominator);
        t.numerator /= dg; t.denominator /= dg;
        return t;
    }
    frac operator/(const frac &o) const { return *this * frac(o.denominator, o.numerator); }
    frac operator-(int o) const { return *this + frac(-o, 1); }
    frac operator-(const frac& o) const { return *this + frac(-o.numerator, o.denominator); }
};
...
int main() {
    ofstream fout(R"(C:\Users\ryans\OneDrive\Desktop\Coding\Baekjoon\z.etcBJ\output.txt)");
    ...
    ans = frac(tar, 1);
    for(int i=0; i<16; i++) {
        ans = ans - (ans * ans - tar) / (ans * 2);
    }
    // 10^1000000 = (10^500000) ^ 2 = 250000 ^ 4 = 125000 ^ 8 = 62500 ^ 16 = 31250 ^ 32 = 15625 ^ 64
    t = BigInt("1" + string(15625, '0'));
    for(int i=0; i<6; i++) t *= t;
    ans.numerator *= t;
